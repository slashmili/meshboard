...
fun main() {
    println(crdtCompat(Json.parseToJsonElement(System.`in`.bufferedReader().readText()).jsonObject))
}

/** Shared by the desktop CLI and Android instrumentation; each request owns a fresh document. */
fun crdtCompat(request: JsonObject): JsonObject {
