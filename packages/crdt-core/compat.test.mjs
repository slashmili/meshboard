...
test('Yjs board fixtures load in yrs; native changes and deletions return to Yjs', t => {
  const doc = web(t, 1)
  const elements = doc.getMap('elements')
  for (const stroke of strokes) elements.set(stroke.id, stroke)
  const loaded = native({ clientId: 2, updates: [snapshot(doc)] })
  deepStrictEqual(loaded.elements, elements.toJSON())
  const extra = { ...strokes[0], id: 'native-stroke', points: [{ x: -1.25, y: 2.75 }] }
  const changed = native({ clientId: 2, updates: [loaded.update], operations: [put(extra), remove(strokes[1].id)], targetStateVector: vector(doc) })
...
test('a fresh Yjs peer loads a large native board and then receives only new changes', t => {
  const large = { ...strokes[0], id: 'large', points: Array.from({ length: 12_000 }, (_, i) => ({ x: i / 8, y: 0 - i / 4 })) }
  const initial = native({ clientId: 10, operations: [put(large)] })
  const newcomer = web(t, 11)
  apply(newcomer, initial.update)
  deepStrictEqual(newcomer.getMap('elements').get('large'), large)
  const changed = native({ clientId: 10, updates: [initial.update], operations: [put(strokes[1])], targetStateVector: vector(newcomer) })
...
test('yrs accepts duplicate and out-of-order Yjs updates without resurrecting deletions', t => {
  const doc = web(t, 20)
  const updates = []
  ...
  const received = native({ clientId: 21, updates: [...updates.toReversed(), ...updates] })
  deepStrictEqual(received.elements, doc.getMap('elements').toJSON())
  strictEqual(Object.keys(received.elements).length, 1)
})

test('Yjs accepts duplicate and out-of-order yrs updates', t => {
  const initial = native({ clientId: 30, operations: [put(strokes[0])] })
  const changed = native({ clientId: 30, updates: [initial.update], operations: [put(strokes[1])], targetStateVector: initial.stateVector })
  const doc = web(t, 31)
...
test('concurrent writes to the same element converge across both implementations', t => {
  const doc = web(t, 40)
  doc.getMap('elements').set(strokes[0].id, strokes[0])
  const initial = snapshot(doc)
  doc.getMap('elements').set(strokes[0].id, { ...strokes[0], color: '#112233' })
  const rust = native({ clientId: 41, updates: [initial], operations: [put({ ...strokes[0], color: '#aabbcc' })] })
  const merged = native({ clientId: 41, updates: [rust.update, snapshot(doc)] })
  apply(doc, rust.update)
  deepStrictEqual(doc.getMap('elements').toJSON(), merged.elements)
})

test('delete-only changes synchronize even when the state vector has not advanced', t => {
  const doc = web(t, 50)
  doc.getMap('elements').set(strokes[0].id, strokes[0])
  const initial = snapshot(doc)
  const before = vector(doc)
  const removed = native({ clientId: 51, updates: [initial], operations: [remove(strokes[0].id)], targetStateVector: before })
...
test('a remaining peer can seed a newcomer after the original creator disappears', t => {
  const creator = web(t, 60)
  creator.getMap('elements').set(strokes[0].id, strokes[0])
  const remaining = native({ clientId: 61, updates: [snapshot(creator)], operations: [put(strokes[1])] })
  ...
  const merged = native({ clientId: 61, updates: [remaining.update, delta] })
